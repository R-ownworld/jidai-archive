...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // ISBNで検索（最優先）
    if (isbn && isbn !== "null") {
      url = `https://app.rakuten.co.jp/services/api/BooksBook/Search/20170404?applicationId=${appId}&isbn=${isbn}&hits=1&formatVersion=2`;
    }
    // タイトル＋著者名で検索
    else if (title) {
      const q = encodeURIComponent(title + (author ? " " + author : ""));
      url = `https://app.rakuten.co.jp/services/api/BooksBook/Search/20170404?applicationId=${appId}&title=${q}&hits=3&formatVersion=2`;
    } else {
    ...
    if (data.Items && data.Items.length > 0) {
      ...
      return NextResponse.json({
        ...
        rakutenUrl: book.itemUrl || null, // 楽天へのリンク（規約上必須）
      }, { headers });
    }
